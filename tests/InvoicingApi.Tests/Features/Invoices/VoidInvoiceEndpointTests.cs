...
public class VoidInvoiceEndpointTests(PostgresFixture postgres)
{
    ...
    [Fact]
    public async Task Voids_sent_invoice()
    {
        ...
        var response = await httpClient.PostAsJsonAsync($"/invoices/{invoice.Id}/void", new VoidInvoiceRequest(invoice.Version));
        var body = await response.Content.ReadFromJsonAsync<InvoiceDto>();

        response.StatusCode.ShouldBe(HttpStatusCode.OK);
        body.ShouldNotBeNull();
        body.Status.ShouldBe(InvoiceStatus.Void);
    }

    [Fact]
    public async Task Returns_conflict_for_draft()
    {
        await using var factory = await CreateFactoryAsync();
    ...
    public async Task Returns_not_found_for_unknown_invoice()
    {
        ...
        var response = await httpClient.PostAsJsonAsync($"/invoices/{Guid.NewGuid()}/void", new VoidInvoiceRequest(0));
    ...
    public async Task Returns_validation_problem_for_negative_version()
    {
        ...
        var response = await httpClient.PostAsJsonAsync($"/invoices/{Guid.NewGuid()}/void", new VoidInvoiceRequest(-1));
