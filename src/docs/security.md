...
- **User self-registration has no path to Admin.** `POST /auth/register` always creates a `User`-role account by design; beyond the seeded `Admin`, promoting someone to `Admin` is currently a manual database update, not an API operation.
